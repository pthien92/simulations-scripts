...
def parseOptions():
    ...
    parser.add_option( '-i', '--interface', dest='interface',
                      default=None, help='External interface' )
...
def testSDNNetworkPerformance(opts):
    ...
    clientSwitch = net.get('s3')
    info( '*** Adding hardware interface', interface, 'to switch',
        clientSwitch.name, '\n' )
    _intf = Intf(interface, node=clientSwitch)
    ...
    h1.cmd('tcpdump -l -e -n port 2000 -i h1-eth0 > /tmp/h1_lte.out &')
    h2.cmd('tcpdump -l -e -n port 2000 -i h2-eth0 > /tmp/h2_lte.out &')
