...
QosController::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::QosController")
    ...
    .AddAttribute ("ServerIpAddr",
                   "Server IPv4 address.",
                   AddressValue (Address (Ipv4Address ("10.1.2.1"))),
...
QosController::ConfigureBorderSwitch (Ptr<const RemoteSwitch> swtch)
{
  ...
  if (m_linkAggregation)
  ...
  else
    {
      ...
      DpctlExecute (swtch, "group-mod cmd=add,type=ind,group=3 "
                    "weight=0,port=any,group=any set_field=ip_src:10.1.2.1"
                    ",set_field=eth_src:00:00:00:00:00:01,output=1");
    }
  ...
  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
                "in_port=1,eth_type=0x0800,ip_proto=6,ip_dst=10.1.2.1,"
                "eth_dst=00:00:00:00:00:01 apply:output=ctrl");
  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
                "in_port=2,eth_type=0x0800,ip_proto=6,ip_dst=10.1.2.1,"
                "eth_dst=00:00:00:00:00:01 apply:output=ctrl");

  // Incoming UDP connections (ports 1 and 2) 
  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
                "in_port=1,eth_type=0x0800,ip_proto=17,ip_dst=10.1.2.1,"
                "eth_dst=00:00:00:00:00:01 apply:group=1");
  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
                "in_port=2,eth_type=0x0800,ip_proto=17,ip_dst=10.1.2.1,"
                "eth_dst=00:00:00:00:00:01 apply:group=2");
  ...
  // UDP packets from servers are sent to the external network through group 3
  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=700 "
                "in_port=3,eth_type=0x0800,ip_proto=17 apply:group=3");
  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=700 "
                "in_port=4,eth_type=0x0800,ip_proto=17 apply:group=3");
...
QosController::GetArpEntry (Ipv4Address ip)
{
  ...
  if (ret != m_arpTable.end ())
    ...
    QosController::SaveArpEntry(ip, ret->second);
    return ret->second;
  }
  // NS_ABORT_MSG ("No ARP information for this IP." << ip << " - " << ret->second);
}
