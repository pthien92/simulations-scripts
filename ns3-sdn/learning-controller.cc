...
ofl_err
LearningController::HandlePacketIn (
  ...
  uint32_t xid)
{
  ...
  if (ethType == Ipv6L3Protocol::PROT_NUMBER) {
    return 0;
  }
  ...
  if (reason == OFPR_NO_MATCH)
    {
      ...
      if (ethType != ArpL3Protocol::PROT_NUMBER) {
        ethSrc = oxm_match_lookup (OXM_OF_ETH_SRC, (struct ofl_match*)msg->match);
      } else {
        // Its an ARP
        ethSrc = oxm_match_lookup (OXM_OF_ARP_SHA, (struct ofl_match*)msg->match);
      }
      ...
      if (ethType != ArpL3Protocol::PROT_NUMBER) {
        ethDst = oxm_match_lookup (OXM_OF_ETH_DST, (struct ofl_match*)msg->match);
      } else {
        ethDst = oxm_match_lookup (OXM_OF_ARP_THA, (struct ofl_match*)msg->match);
      }
      ...
      if (it != m_learnedInfo.end ())
        {
          ...
          if (itSrc == l2Table->end ())
            ...
            {
              // NS_LOG_INFO("Error happened when looking up macSrc=" << src48);
...
ofl_err
LearningController::HandleFlowRemoved (
...
/********** Private methods **********/
void
LearningController::HandshakeSuccessful (
